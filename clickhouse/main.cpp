...
void callbak_func(const Block &block, std::string quers)
{
    for (Block::Iterator bi(block); bi.IsValid(); bi.Next())
    {
        std::cout << bi.Name() << " ";
    ...
    for (size_t i = 0; i < block.GetRowCount(); ++i)
    {
        std::cout << (*block[0]->As<ColumnInt32>())[i] << " "
                  << (*block[1]->As<ColumnUInt64>())[i] << " "
                  << (*block[2]->As<ColumnUInt32>())[i] << "\n";
    }
}
...
static void test(Client &client)
{
    ...
    /// Create a table.
    /*client.Execute("select count(*) from ncdatas.test_table");
    ...
    client.Select("select datatime, app_id, src_ip from ncdatas.test_table limit 10", callbak_func);
...
int main()
{
    ...
    {
        {
            Client client(ClientOptions().SetHost("192.168.0.232").SetPingBeforeQuery(true).SetDefaultDatabase("ncdatas").SetUser("ncompass").SetPassword("3edc2wsx1qaz"));
            RunTests(client);
            //test(client);
        }
