...
bool Client::Impl::ReceiveData() {
    ...
    if (REVISION >= DBMS_MIN_REVISION_WITH_TEMPORARY_TABLES) {
        ...
        if (!WireFormat::ReadString(&input_, &table_name)) {
			//std::cout<<"table_name error = "<<std::endl;
            return false;
        }
...
void Client::Select(const std::string& query, SelectCallback cb, char *data) {
	Execute(Query(query).SetOnData(cb, data));
}
