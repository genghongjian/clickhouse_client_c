...
#define DECLARE_FIELD(name, type, setter, default) \
    ...
    /// Default database.
    DECLARE_FIELD(default_database, std::string, SetDefaultDatabase, "ncdatas");
    /// User name.
    DECLARE_FIELD(user, std::string, SetUser, "ncompass");
    /// Access password.
    DECLARE_FIELD(password, std::string, SetPassword, "3edc2wsx1qaz");
...
public:
    ...
    void Select(const std::string& query, SelectCallback cb, char *data);
